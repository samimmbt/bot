...
async def at_set_markup_menu(_, query: CallbackQuery):
    ...
    if not a.can_manage_voice_chats:
        return await query.answer("💡 ادمین هایی با دسترسی لازمه میتوانند این دکمه را کلیک کنند !", show_alert=True)
    chat_id = query.message.chat.id
    user_id = query.message.from_user.id
    buttons = menu_markup(user_id)
    if chat_id in QUEUE:
        await query.answer("کنترل پنل باز شد")
        await query.edit_message_reply_markup(reply_markup=InlineKeyboardMarkup(buttons))
    else:
        await query.answer("❌ چیزی استریم نمی شود", show_alert=True)
...
async def is_set_home_menu(_, query: CallbackQuery):
    ...
    if not a.can_manage_voice_chats:
        return await query.answer("💡 ادمین هایی با دسترسی لازمه میتوانند این دکمه را کلیک کنند !", show_alert=True)
    await query.answer("کنترل پنل بسته شد")
    user_id = query.message.from_user.id
